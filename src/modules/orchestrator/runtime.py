...
class StageAgentRuntime:
    ...
    def build_system_prompt(self, state: Dict, base_system_prompt: str, kb_items: List[Dict], bound_ai_id: str = None) -> str:
        ...
        try:
            ...
            if skills:
                ...
                for sk in skills:
                    ...
                    parts.append(f"- [{sk.get('name') or ''}] {desc}".rstrip())
                    tpl = (cfg.get("template") or "").strip()
                    if tpl:
                        parts.append(f"  规则/Prompt: {tpl}")
        except Exception:
            pass
