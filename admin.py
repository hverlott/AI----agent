...
def start_bot():
    ...
    try:
        ...
        archive_log_file(BOT_LOG_FILE, LOG_ARCHIVE_DIR, "bot")
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        if st.button("🔎 环境诊断", use_container_width=True):
            missing, invalid = diagnose_env()
            if not missing and not invalid:
                st.success("✅ 环境变量配置完整")
            else:
        ...
        def render_log_tab(tab_label, file_path, prefix, key_prefix):
            ...
            with col1:
                if st.button("\u5237\u65b0", use_container_width=True, key=f"{key_prefix}_refresh"):
                    st.rerun()
            with col2:
                if st.button("\u5b58\u6863", use_container_width=True, key=f"{key_prefix}_archive"):
                    archived = archive_log_file(file_path, LOG_ARCHIVE_DIR, prefix)
                    if archived:
                        st.success("\u5df2\u5b58\u6863")
            with col3:
                if st.button("\u5b58\u6863\u5e76\u6e05\u7a7a", use_container_width=True, key=f"{key_prefix}_archive_clear"):
