...
PLATFORMS = {
    ...
    'orchestrator': {
        ...
        'color': '#3b82f6',
        'status': 'available',
        'description': 'Stage/Persona/Binding',
        'roles': ['SuperAdmin']
    },
    'supervisor': {
        'name': 'Supervisor',
        ...
        'color': '#22c55e',
        'status': 'available',
        'description': 'Sessions & Decisions',
        'roles': ['SuperAdmin']
    },
    'help_center': {
        'name': '帮助中心',
...
def render_system_status_panel():
    ...
    nodes = [
        ("Channel Ingress","MSG_RECEIVED"),
        ("State Load","STATE_LOADED"),
        ("Supervisor Decide","SUPERVISOR_DECIDED"),
        ("KB Retrieve","KB_RETRIEVED"),
        ("Stage Agent Generate","STAGE_AGENT_GENERATED"),
        ("Style Guard","STYLE_GUARD"),
        ("Audit / Keyword","AUDIT_RESULT"),
        ("Reply Send","REPLY_SENT"),
...
def save_platform_config(platform, config):
    ...
    try:
        log_admin_op("platform_config_save", {"platform": platform})
    except Exception:
        pass
...
def render_platform_selector():
    ...
    groups = [
        ("🛠️ 系统管理", ["sys_config", "api_gateway", "ai_config", "accounts"]),
        ("💼 业务管理", ["business", "orchestrator", "telegram", "whatsapp", "facebook", "messenger", "wechat", "instagram", "twitter", "discord"]),
        ("📚 数据管理", ["knowledge"]),
        ("📡 监控中心", ["supervisor", "system_status", "audit"]),
        ("🆘 文档与帮助", ["help_center"])
    ]
    for g_title, g_items in groups:
        with st.sidebar.expander(g_title, expanded=selected_platform in g_items):
            for platform_id in g_items:
                ...
                if roles and current_role != 'SuperAdmin':
                    continue
                icon = info.get('icon', '')
...
def render_telegram_panel():
    ...
    with col2:
        if os.path.exists("userbot_session.session"):
            st.success(tr("tg_login_success"))
        else:
            st.warning(tr("tg_login_warning"))
            if st.button(tr("tg_login_btn"), use_container_width=True, key="tg_login"):
    ...
    with col2:
        if st.button(tr("tg_btn_stop"), use_container_width=True, 
                    disabled=not is_running, key="tg_stop"):
            with st.spinner("正在停止机器人…"):
    ...
    with col3:
        if st.button(tr("tg_btn_restart"), use_container_width=True,
                    disabled=False, key="tg_restart"):
            with st.spinner("正在重启机器人…"):
                if is_running:
                    stop_success, stop_msg = stop_bot()
                    if not stop_success:
                        st.warning(f"停止失败: {stop_msg}")
                    import time
                    time.sleep(1)
                start_success, start_msg = start_bot()
            if start_success:
...
def render_telegram_config():
    ...
    with col1:
        ...
        if st.button(tr("tg_cfg_save_prompt"), key="save_prompt"):
            ...
            log_admin_op("tg_prompt_save", {"path": prompt_path})
            st.success(tr("common_success"))
    ...
    with col2:
        ...
        if st.button(tr("tg_cfg_save_keywords"), key="save_keywords"):
            ...
            log_admin_op("tg_keywords_save", {"path": keywords_path})
            st.success(tr("common_success"))
    ...
    with col1:
        ...
        orchestration_enabled = st.toggle(
            "🧠 AI 会话编排引擎 (Orchestration Engine)",
    ...
    with col_tmp1:
        if st.button(tr("tg_cfg_audit_temp_off"), key="tg_audit_temp_off", use_container_width=True):
            ...
            st.success(tr("common_success"))
    with col_tmp2:
        if st.button(tr("tg_cfg_audit_restore"), key="tg_audit_restore", use_container_width=True):
    ...
    try:
        ...
        if until and _time.time() > until:
            ...
            st.success("Audit Auto Restored")
    except Exception:
        pass
    ...
    if st.button(tr("tg_cfg_save_all"), use_container_width=True):
        new_config = f"""# ========================================
...
"""
        ...
        log_admin_op("tg_config_save", {"AUTO_QUOTE": auto_quote, "PRIVATE_REPLY": private_reply, "GROUP_REPLY": group_reply})
        st.success(tr("common_success"))
    ...
    if st.button(tr("tg_kw_save_fallback"), key="save_audit_fallback", use_container_width=True):
        write_file(fallback_path, fallback_text)
        log_admin_op("tg_fallback_save", {"path": fallback_path})
        st.success(tr("common_success"))

    st.markdown(tr("tg_kw_clean_qa"))
    qa_path = os.path.join("platforms", "telegram", "qa.txt")
    if st.button(tr("tg_kw_clean_btn"), key="tg_qa_clean", use_container_width=True):
        ...
        for ln in lines:
            ...
            cleaned.append(s)
        write_file(qa_path, "\n".join(cleaned))
        log_admin_op("tg_qa_clean", {"removed": removed})
    ...
    if st.button(tr("tg_whitelist_save"), key="save_tg_whitelist", use_container_width=True):
...
def render_telegram_broadcast():
    ...
    if st.button(tr("tg_bc_send_btn"), type="primary", use_container_width=True, key="tg_broadcast_send"):
        if not selected_chat_ids:
            ...
            try:
                ...
                log_file = os.path.join("platforms", "telegram", "logs", "broadcast.log")
    ...
    if os.path.exists(log_file):
        ...
        # Duplicate residual block removed
...
def render_telegram_logs():
    ...
    with log_tab1:
        render_log_tab(tr("tg_log_sys"), TG_SYSTEM_LOG_FILE, "tg_log_system")
    with log_tab2:
        render_log_tab(tr("tg_log_priv"), TG_PRIVATE_LOG_FILE, "tg_log_private")
    with log_tab3:
        render_log_tab(tr("tg_log_grp"), TG_GROUP_LOG_FILE, "tg_log_group")
    with log_tab4:
        render_log_tab(tr("tg_log_audit"), os.path.join("platforms", "telegram", "logs", "audit.log"), "tg_log_audit")
...
def render_orchestrator_panel():
    ...
    with tabs[0]:
        st.subheader("Stage 流程配置")
        name = st.selectbox("选择或创建 Stage", ["S0","S1","S2","S3","S4","S5"], key="orch_stage_name_sel")
    ...
    with tabs[2]:
        ...
        content = st.text_area(tr("orch_binding_content"), value=init, height=180, key="orch_binding_content")
    ...
    with tabs[5]:
        st.subheader("🛡️ Style Guard 配置")
        st.caption("配置 AI 回复的后处理规则，防止机器人味过重。")
    ...
    with tabs[6]:
        ...
        if st.button("开始检查", key="health_check_btn"):
            ...
            if not os.getenv("AI_API_KEY"):
                issues.append("❌ 未配置 AI_API_KEY (环境变量)")
            
            # 2. 检查 Telegram Config
            tg_conf_path = "platforms/telegram/config.txt"
            if not os.path.exists(tg_conf_path):
                issues.append(f"❌ 缺少配置文件: {tg_conf_path}")
            else:
                try:
                    with open(tg_conf_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                        if "CONV_ORCHESTRATION=on" not in content:
                            issues.append("⚠️ 编排模式未开启 (CONV_ORCHESTRATION=off)")
                except:
                    pass
...
def render_ai_config_panel():
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        provider = st.selectbox(tr("ai_provider"), ["DeepSeek", "OpenAI", "AzureOpenAI", "LocalAI"], key="ai_provider")
    ...
    with col3:
        ...
        timeout = st.number_input(tr("ai_timeout"), min_value=1, max_value=60, value=10, step=1, key="ai_timeout")
...
def render_api_gateway_panel():
    ...
    if st.button(tr('api_btn_add'), use_container_width=True, key="gw_add"):
...
def _vault_paths(tenant):
    ...
    return {
        ...
        "user_session": os.path.join(BASE_DIR, "userbot_session.session"),
        "admin_session": os.path.join(BASE_DIR, "admin_session.session"),
    }
...
def _get_crypto():
    try:
        ...
        return getattr(mod, "Fernet", None)
    except Exception:
        return None
...
def _decrypt(ciphertext, key_bytes):
    Fernet = _get_crypto()
    if Fernet:
        f = Fernet(key_bytes)
...
def render_sys_config_panel():
    ...
    with colA:
        ...
        if st.button(tr('sys_btn_gen_env'), use_container_width=True, key="btn_gen_env"):
            try:
                backup_dir = _ensure_data_dirs()
                backup_dir = os.path.join(backup_dir, "backups", "auto-rollback")
                os.makedirs(backup_dir, exist_ok=True)
                ts = datetime.now().strftime("%Y%m%d-%H%M%S")
                if os.path.exists(paths["env"]):
                    shutil.copy2(paths["env"], os.path.join(backup_dir, f".env.{ts}.bak"))
                tmp_path = paths["env"] + ".tmp"
                content = [
...
def render_whatsapp_config():
    ...
    with col1:
        ...
        if st.button(tr('wa_cfg_save_prompt'), key="wa_save_prompt"):
            ...
            log_admin_op("wa_prompt_save", {"path": "platforms/whatsapp/prompt.txt"})
            st.success(tr('common_success'))
    ...
    with col2:
        ...
        if st.button(tr('wa_cfg_save_keywords'), key="wa_save_keywords"):
            ...
            log_admin_op("wa_keywords_save", {"path": "platforms/whatsapp/keywords.txt"})
            st.success(tr('common_success'))
    ...
    if st.button(tr('wa_cfg_save_config'), key="wa_save_config"):
        ...
        log_admin_op("wa_config_save", {"private_reply": private_enabled, "group_reply": group_enabled})
        st.success(tr('common_success'))
...
def main():
    ...
    # --- Role Selection (Hardcoded to SuperAdmin) ---
    if 'user_role' not in st.session_state:
        st.session_state.user_role = 'SuperAdmin' 
    st.session_state.user_role = 'SuperAdmin' # Enforce
