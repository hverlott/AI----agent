...
- **知识库引擎 (Knowledge Base Engine)**:
  - **智能去重与覆盖**: 上传同名文件时自动覆盖旧版本条目，确保知识库内容实时更新且无冗余。
  - **大文件分块优化**: 针对 PDF 文档引入带重叠的分块策略 (Chunking with Overlap)，有效解决跨页内容截断问题，提升检索准确率。
  - **持久化稳定性**: 修复了系统重启或重载时，非预置 (Fallback) 的用户上传条目意外丢失的问题。

- **对话表现控制 (Conversation Control)**:
  - **官方客服模式强化**: 修复了 `CONVERSATION_MODE=ai_visible` 在 KB 直答模式下未生效的问题。现在官方模式会严格执行“结论优先、无口语铺垫、限长200字”的约束。
  - **拟人化区分**: 明确区分了“官方技术支持”与“真人模拟”两种回复风格，确保机器人人设与后台配置完全一致。
