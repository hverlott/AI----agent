#!/usr/bin/env node
/**
 * WhatsApp AI Bot - 自动回复机器人 (多租户适配版)
...
if (TENANT_ID) {
    // 如果提供了 TENANT_ID，则所有数据文件指向租户目录
    // 假设当前目录是 platforms/whatsapp
    // 目标目录是 data/tenants/{TENANT_ID}/platforms/whatsapp
    BASE_DIR = path.resolve(__dirname, '../../data/tenants', TENANT_ID, 'platforms/whatsapp');
    ...
    if (!fs.existsSync(BASE_DIR)) {
        try {
            ...
            console.error(`无法创建租户目录: ${BASE_DIR}`, e);
            // Fallback to __dirname if creation fails
            BASE_DIR = __dirname;
        }
    }
    console.log(`🔒 租户隔离模式已启用: ${TENANT_ID}`);
    console.log(`📂 工作目录: ${BASE_DIR}`);
} else {
    console.log('⚠️ 未提供 TENANT_ID，运行在默认模式 (共享目录)');
}
// ====================================================
...
// 加载统计数据
function loadStats() {
    const statsPath = path.join(BASE_DIR, 'stats.json');
    const defaultStats = {
        total_messages: 0,
        total_replies: 0,
        private_messages: 0,
...
function loadConfig() {
    try {
        // 读取功能开关
        const configPath = path.join(BASE_DIR, 'config.txt');
        ...
        // 读取关键词
        const keywordsPath = path.join(BASE_DIR, 'keywords.txt');
...
async function main() {
    ...
    // 确定 Session 路径
    // 如果有 TENANT_ID，则存放在租户目录下的 sessions 子目录
    // 否则存放在当前目录的 .wwebjs_auth
    const authOptions = {
        clientId: 'whatsapp-ai-bot'
    };
    
    if (TENANT_ID) {
        authOptions.dataPath = path.join(BASE_DIR, 'sessions');
    }
    ...
    client.on('qr', async (qr) => {
        ...
        try {
            const qrImagePath = path.join(BASE_DIR, 'qr_code.png');
            ...
            // 创建状态文件
            const statusPath = path.join(BASE_DIR, 'login_status.json');
            ...
            console.log(`✅ 二维码已保存到 ${qrImagePath}（可在 Web 管理后台查看）\n`);
        } catch (error) {
    ...
    client.on('message', async (msg) => {
        try {
            ...
            // 调用 AI
            console.log('🤖 AI 正在思考...');
            let reply = await getAIReply(msg.body, history);
            
            if (reply) {
                // ========== 双重审计接入 ==========
                try {
                    const auditUrl = process.env.AUDIT_API_URL || "http://localhost:8000/audit";
                    const auditRes = await axios.post(auditUrl, {
                        user_input: msg.body,
                        draft_reply: reply,
                        history: history
                    });
                    
                    if (auditRes.data && !auditRes.data.approved) {
                        console.log(`🛑 审计拦截: ${auditRes.data.reason}`);
                        reply = auditRes.data.suggestion || "很抱歉，我无法回答这个问题。";
                    } else {
                        console.log('✅ 审计通过');
                    }
                } catch (error) {
                    console.error("⚠️ 审计服务不可用，跳过:", error.message);
                }
                // ==================================
