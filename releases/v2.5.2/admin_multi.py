...
# 页面配置
# Fetch system config
website_title = db.get_system_config("website_title", "SaaS AI System v2.5.2")

# Force update title if it's the old default
current_title = db.get_system_config("website_title", "SaaS AI System v2.5.2")
if "v2.5.1" in current_title:
    db.set_system_config("website_title", "SaaS AI System v2.5.2")
    current_title = "SaaS AI System v2.5.2"
...
if "SAAS_DATA_DIR" in os.environ:
    DATA_DIR = Path(os.environ["SAAS_DATA_DIR"])
elif (BASE_DIR.parent.name == "releases" or BASE_DIR.parent.name.startswith("v")) and (BASE_DIR.parent.parent / "data").exists():
...
def tr(key: str) -> str:
    ...
    val = I18N.get(lang, I18N["zh"]).get(key, key)
...
def _ensure_vault_key(key_path):
    """确保存在 Vault Key，如果不存在则生成"""
    try:
...
def _encrypt(text, key):
    """简易加密 (XOR + Base64)"""
    if not text: return ""
    if not key: return text # Fallback
    try:
...
def _decrypt(encrypted_text, key):
    ...
    if not key: return encrypted_text
    try:
...
def get_tenant_path(rel_path, tenant_id=None):
    """获取租户文件路径"""
...
def render_system_status_panel():
    ...
    st.number_input("自动刷新间隔(ms)", value=10000, step=1000)
    if st.button("手动刷新"):
        try:
            st.rerun()
        except Exception:
...
def load_platform_config(platform):
    ...
    tenant_config_file = DATA_DIR / "tenants" / tenant_id / "platforms" / platform / "config.json"
...
def save_platform_config(platform, config):
    ...
    config_file = DATA_DIR / "tenants" / tenant_id / "platforms" / platform / "config.json"
...
def _get_tenant_kb_dirs(tenant_id):
    """获取租户专属的知识库目录"""
    base = os.path.join(DATA_DIR, "tenants", tenant_id, "knowledge_base")
...
def render_kb_panel():
    ...
    with tabs[0]:
        # st.subheader(tr("kb_list_title")) # Moved to expander
        # DB Migration: Use SQLite instead of JSON
        items = db.get_kb_items(tenant_id)
        
        with st.expander(f"📚 {tr('kb_list_title')}", expanded=True):
            if not items:
        ...
        st.divider()
        with st.expander(tr("kb_new_item"), expanded=True):
        ...
        if "kb_edit_id" in st.session_state and st.session_state.kb_edit_id:
            with st.expander(f"✏️ {tr('kb_edit_title')}", expanded=True):
    ...
    with tabs[1]:
        # st.subheader(tr("kb_import_header")) # Moved to expander
        with st.expander(f"📤 {tr('kb_import_header')}", expanded=True):
            uploaded = st.file_uploader(tr("kb_import_desc"), type=["txt","md","pdf","docx","xlsx"], key="kb_file_uploader")
            if uploaded:
                safe_name = uploaded.name
                # Use tenant specific dir for isolation
                tenant_files_dir = _get_tenant_kb_dirs(tenant_id)
                dest_path = os.path.join(tenant_files_dir, safe_name)
    ...
    with tabs[2]:
        ...
        with st.expander("🧪 测试查询", expanded=True):
    ...
    with tabs[3]:
        with st.expander(f"⚙️ {tr('kb_settings_header')}", expanded=True):
            st.caption("Optional dependencies: pypdf, python-docx, openpyxl.")
            missing = []
            try: import pypdf
            except: missing.append("pypdf")
            try: import docx
...
def render_tenant_login_panel(tenant_id, session_name):
    ...
    with st.expander(f"🔐 登录 Telegram: {session_name}", expanded=True):
...
def render_telegram_panel():
    ...
    if os.path.exists(api_db_path):
        try:
            with open(api_db_path, "r", encoding="utf-8") as f:
                ...
                api_configs = [a for a in api_data.get("api_configs", []) if a.get("platform") == "Telegram"]
        except:
            pass
    ...
    with st.expander("🎯 运行配置选择 (Run Configuration)", expanded=True):
        ...
        with sel_col1:
            if api_configs:
                api_options = {}
                default_api_idx = 0
                for idx, conf in enumerate(api_configs):
                    label = f"{conf.get('name', 'Unnamed')} (ID: {conf.get('api_id')})"
                    api_options[label] = conf
                    if str(conf.get("api_id")) == str(active_api_id):
                        default_api_idx = idx
                
                selected_api_label = st.selectbox(
                    "🤖 选择 Bot API 配置", 
                    list(api_options.keys()), 
                    index=default_api_idx if default_api_idx < len(api_options) else 0,
                    key="tg_panel_api_sel"
                )
                if selected_api_label:
                    selected_api_conf = api_options[selected_api_label]
                    
                    # 如果选中的跟当前生效的不一致，显示应用按钮
                    if str(selected_api_conf.get("api_id")) != str(active_api_id):
                        if st.button("🚀 应用此 API 配置", key="tg_apply_api_btn"):
                            os.makedirs(t_conf_dir, exist_ok=True)
                            final_conf = {}
                            if os.path.exists(active_cfg_path):
                                try:
                                    with open(active_cfg_path, 'r') as f:
                                        final_conf = json.load(f)
                                except: pass
                            
                            final_conf.update({
                                "api_id": selected_api_conf["api_id"],
                                "api_hash": selected_api_conf["api_hash"],
                                "bot_token": selected_api_conf.get("bot_token", "")
                            })
                            
                            with open(active_cfg_path, "w", encoding="utf-8") as f:
                                json.dump(final_conf, f, ensure_ascii=False, indent=2)
                            st.toast("✅ API 配置已切换并应用！")
                            import time
                            time.sleep(1)
                            st.rerun()
            else:
                st.warning("⚠️ 未检测到 API 配置，请先前往【账号管理】添加。")
        ...
        with sel_col2:
            if tg_accounts:
                ...
                selected_label = st.selectbox("👤 选择 Telegram 账号", list(account_options.keys()), key="tg_panel_acc_sel")
    ...
    # 状态显示
    with st.expander("📊 运行状态监控 (Status Monitor)", expanded=True):
...
def render_telegram_config():
    ...
    with st.expander(tr("tg_cfg_persona"), expanded=True):
    ...
    with st.expander(tr("tg_cfg_qa"), expanded=False):
    ...
    # --- API 密钥配置 (已移除，移至【业务管理-账号管理】) ---
    # st.markdown("### 🔑 API 密钥配置 (SaaS)")
    # st.info("SaaS 模式下，请在此配置您的 API 密钥，而不是依赖系统环境变量。")
    
    st.divider()
    
    # 功能开关与参数
    with st.expander(tr("tg_cfg_switches"), expanded=True):
            ...
            if '=' in line and not line.strip().startswith('#'):
                ...
                if len(parts) == 2:
                    ...
                    if key in ['PRIVATE_REPLY', 'GROUP_REPLY', 'AUDIT_ENABLED', 'AUTO_QUOTE', 'CONV_ORCHESTRATION', 'KB_ONLY_REPLY', 'SENDER_FILTER_ENABLED']:
                        ...
                        try:
                            current_config[key] = json.loads(raw_value)
                        except:
                            current_config[key] = {"enabled": False, "rules": []}
        ...
        st.markdown("---")
        with st.expander("🛡️ 内容过滤配置 (Content Filter)", expanded=False):
            st.caption("配置关键词过滤规则，当消息中包含特定关键词且占比超过阈值时进行拦截。")
            
            # Load config
            filter_conf = current_config.get("CONTENT_FILTER", {})
            if not isinstance(filter_conf, dict):
                filter_conf = {"enabled": False, "rules": []}
            
            filter_enabled = st.toggle("启用内容过滤", value=filter_conf.get("enabled", False), key="tg_filter_enabled")
            
            st.markdown("#### 过滤规则配置")
            st.caption("简化模式：全局统一阈值，关键词用逗号分隔。")
            
            filter_keywords = st.text_area(
                "过滤关键词列表",
                value=filter_conf.get("keywords", ""),
                placeholder="例如：加群, 兼职, 刷单",
                help="输入需过滤的敏感词，多个词之间用逗号(,)分隔",
                key="tg_filter_keywords",
                height=100
            )
            ...
            filter_threshold = st.slider(
                ...
                min_value=0.1,
                max_value=1.0,
                value=float(filter_conf.get("threshold", 0.8)),
                step=0.05,
                help="如果关键词在消息内容中的字符占比超过此阈值，则触发拦截。值越低越严格。"
            )
            
            # Update config object in session
            current_config["CONTENT_FILTER"] = {
                "enabled": filter_enabled,
                "keywords": filter_keywords,
                "threshold": filter_threshold
            }
            
            # Count effective keywords
            kw_count = len([k for k in filter_keywords.split(',') if k.strip()])
            st.info(f"当前生效关键词数: {kw_count}")
    
    st.divider()
    
    with st.expander(tr("tg_cfg_quote"), expanded=False):
    ...
    # AI 温度配置
    with st.expander(tr("tg_cfg_temp"), expanded=False):
    ...
    # Audit Configuration
    with st.expander("🛡️ " + tr("tg_cfg_audit"), expanded=False):
        # st.markdown(tr("tg_cfg_audit")) # Moved to expander title
...
# 内容过滤配置 (JSON)
CONTENT_FILTER={json.dumps(current_config.get("CONTENT_FILTER", {}), ensure_ascii=False)}
...
"""
    ...
    with st.expander("🔑 " + tr("tg_cfg_audit_kw"), expanded=False):
        ...
        from src.modules.audit.keyword_manager import KeywordManager
...
def render_telegram_broadcast(active_api_conf=None, active_acc_conf=None):
    ...
    if st.session_state.get("tg_broadcast_groups"):
        with st.expander("✉️ 消息内容与发送 (Message & Send)", expanded=True):
            col_int, _ = st.columns([1, 2])
            with col_int:
    ...
    st.divider()
    with st.expander("📜 发送记录 (History)", expanded=True):
        log_file = _get_tenant_tg_paths(tenant_id)["broadcast_log"]
...
def render_telegram_logs():
    ...
    def render_log_tab(tab_label, file_path, key_prefix):
        ...
        if not logs:
            st.info(tr("tg_log_tip_load"), icon="ℹ️")
        
        # Display logs in code block for better readability
        st.code(logs if logs else "No logs available.", language="text", line_numbers=True)
...
def render_telegram_flow():
    ...
    with st.expander("🌊 业务流程图解 (Workflow Diagram)", expanded=True):
...
def render_accounts_panel():
    ...
    # 2. Bot API Configs DB
    api_db_path = os.path.join(tdir, "api_configs.json")
    try:
        if os.path.exists(api_db_path):
            with open(api_db_path, "r", encoding="utf-8") as f:
                api_db = json.load(f)
    ...
    # Top-level Tabs
    main_tabs = st.tabs(["👤 社媒账号管理", "🤖 社媒机器人API管理"])

    # ==================== Tab 1: User Accounts ====================
    with main_tabs[0]:
        st.markdown(tr("acc_subtitle"))
        sub_tabs = st.tabs(["📝 手动添加", "📂 批量导入 (Session)", "📋 账号列表"])

        with sub_tabs[0]:
            with st.expander("➕ " + tr("acc_btn_add"), expanded=True):
                col1, col2 = st.columns(2)
                with col1:
        ...
        with sub_tabs[1]:
            ...
            with st.expander("📂 导入设置", expanded=True):
                ...
                if uploaded_files and st.button("开始导入", type="primary", width="stretch"):
                    ...
                    for up_file in uploaded_files:
                        ...
                        # Save file
                        # safe_fname = f"{f_platform}_{f_username}_{fname}" 
                        ...
                        if f_platform == "Telegram" and fname.endswith(".session"):
                            api_id = os.getenv("TELEGRAM_API_ID")
                            api_hash = os.getenv("TELEGRAM_API_HASH")
                            # Try to use tenant config first if available
                            t_cfg_path = os.path.join(tdir, "platforms", "telegram", "config.json")
                            if os.path.exists(t_cfg_path):
                                try:
                                    with open(t_cfg_path, 'r') as tf:
                                        t_cfg = json.load(tf)
                                        if t_cfg.get("api_id") and t_cfg.get("api_hash"):
                                            api_id = t_cfg["api_id"]
                                            api_hash = t_cfg["api_hash"]
                                except: pass
        ...
        with sub_tabs[2]:
            ...
            if disp_accounts:
                ...
                if not selected_rows.empty:
                    with st.expander("🛠️ 批量操作", expanded=True):
                        ...
                        with c1:
                            if st.button("🔍 验证有效性", type="secondary", key="acc_batch_verify"):
                                ...
                                # Try to load API creds from tenant config
                                api_id = os.getenv("TELEGRAM_API_ID")
                                api_hash = os.getenv("TELEGRAM_API_HASH")
                                t_cfg_path = os.path.join(tdir, "platforms", "telegram", "config.json")
                                if os.path.exists(t_cfg_path):
                                ...
                                if not api_id or not api_hash:
                                    st.error("请先在【社媒机器人API管理】中配置并应用一个 API 密钥")
                                else:
    ...
    # ==================== Tab 2: Bot APIs ====================
    with main_tabs[1]:
        st.markdown("### 🤖 社媒机器人 API 管理")
        st.caption("在此管理 Telegram API ID, Hash, Bot Token。可添加多个配置，并选择其中一个应用到当前租户。")
        
        api_sub_tabs = st.tabs(["📝 手动添加 API", "📂 批量导入", "📋 API 列表 (启用/管理)"])
        
        with api_sub_tabs[0]:
            with st.expander("➕ 添加新 API 配置", expanded=True):
                with st.form("add_api_form"):
                    c1, c2 = st.columns(2)
                    api_platform = c1.selectbox("平台", ["Telegram"], key="api_platform")
                    api_name = c2.text_input("备注名称 (如: 营销号1)", key="api_name")
                    
                    st.markdown("#### 🔑 基础凭证")
                    c3, c4 = st.columns(2)
                    api_id = c3.text_input("API ID", key="api_id")
                    api_hash = c4.text_input("API Hash", key="api_hash")
                    bot_token = st.text_input("Bot Token (可选，仅 Bot 模式需要)", key="bot_token")
                    
                    if st.form_submit_button("保存 API 配置"):
                        if not api_id or not api_hash:
                            st.error("API ID 和 API Hash 不能为空")
                        else:
                            new_api = {
                                "id": str(int(datetime.now().timestamp())),
                                "platform": api_platform,
                                "name": api_name or f"API_{api_id}",
                                "api_id": api_id,
                                "api_hash": api_hash,
                                "bot_token": bot_token,
                                "created_at": datetime.now().isoformat()
                            }
                            api_db["api_configs"].append(new_api)
        ...
        with api_sub_tabs[1]:
            with st.expander("📂 批量导入", expanded=True):
                st.info("支持导入 JSON 格式的 API 配置列表。")
                st.markdown("""
                **JSON 格式示例:**
                ```json
                [
                ...
                ```
                """)
                api_file = st.file_uploader("选择 JSON 文件", type=["json"], key="api_uploader")
                if api_file and st.button("导入 API 配置"):
                    try:
                        data = json.load(api_file)
                        if isinstance(data, list):
                            count = 0
                            for item in data:
                                if item.get("api_id") and item.get("api_hash"):
                                    item["id"] = item.get("id") or str(int(datetime.now().timestamp()) + count)
                                    item["created_at"] = datetime.now().isoformat()
                                    api_db["api_configs"].append(item)
                                    count += 1
                            with open(api_db_path, "w", encoding="utf-8") as f:
                                json.dump(api_db, f, ensure_ascii=False, indent=2)
                            st.success(f"成功导入 {count} 条配置！")
                            st.rerun()
                        else:
                            st.error("JSON 格式错误: 根元素必须是列表")
                    except Exception as e:
                        st.error(f"解析错误: {e}")

        with api_sub_tabs[2]:
            if not api_db["api_configs"]:
                st.info("暂无 API 配置，请先添加。")
            else:
                # Load current active config to mark it
                active_cfg_path = os.path.join(tdir, "platforms", "telegram", "config.json")
                active_id = None
                try:
                    if os.path.exists(active_cfg_path):
                        with open(active_cfg_path, 'r') as f:
                            c = json.load(f)
                            active_id = c.get("api_id") # Use api_id as identifier for now
                except: pass

                for idx, conf in enumerate(api_db["api_configs"]):
                    is_active = str(conf.get("api_id")) == str(active_id)
                    icon = "✅" if is_active else "🤖"
                    title = f"{icon} {conf.get('platform')} - {conf.get('name')} (ID: {conf.get('api_id')})"
                    with st.expander(title, expanded=is_active):
                        c1, c2 = st.columns([3, 1])
                        with c1:
                            st.text(f"API Hash: {conf.get('api_hash')[:6]}***")
                            if str(conf.get("api_id")) == str(active_id):
                                st.markdown("✅ **当前已应用**")
                        with c2:
                            if st.button("🚀 应用此配置", key=f"apply_{idx}"):
                                # Write to platforms/telegram/config.json
                                t_conf_dir = os.path.join(tdir, "platforms", "telegram")
                                os.makedirs(t_conf_dir, exist_ok=True)
                                target_file = os.path.join(t_conf_dir, "config.json")
                                
                                # Preserve other settings in target file if exists
                                final_conf = {}
                                ...
                                # Update with selected credentials
                                final_conf.update({
                                    "api_id": conf["api_id"],
                                ...
                                with open(target_file, "w", encoding="utf-8") as f:
                                    json.dump(final_conf, f, ensure_ascii=False, indent=2)
                                st.success("✅ 配置已应用！请去控制面板重启机器人。")
                                st.rerun()
                            
                            if st.button("🗑️ 删除", key=f"del_api_{idx}"):
                                del api_db["api_configs"][idx]
                                with open(api_db_path, "w", encoding="utf-8") as f:
                                    json.dump(api_db, f, ensure_ascii=False, indent=2)
                                st.rerun()
...
def render_orchestrator_panel():
    ...
    with tabs[0]:
        st.subheader("流程阶段配置")
        with st.expander("⚙️ 阶段与版本选择", expanded=True):
        ...
        def _render_stage_flowchart(struct):
            try:
                nodes = struct.get("nodes") or []
                trans = struct.get("transitions") or []
                
                dot = ['digraph StageFlow {']
                dot.append('  rankdir="LR";')
                dot.append('  node [shape=box, style="rounded,filled", fillcolor="#FFFFFF", fontname="Helvetica"];')
                ...
                for n in nodes:
                    ...
                    color = ""
                    if ntype == "start": 
                        color = 'fillcolor="#90EE90"' # LightGreen
                    elif ntype == "end": 
                        color = 'fillcolor="#FFB6C1"' # LightPink
                    elif ntype == "normal":
                        color = 'fillcolor="#E0E0E0"'
                    
                    dot.append(f'  "{nid}" [label="{label}" {color}];')
                    
                for t in trans:
                    src = t.get("from")
                    dst = t.get("to")
                    cond = t.get("condition", "")
                    label = f' [label="{cond}"]' if cond else ""
                    dot.append(f'  "{src}" -> "{dst}"{label};')
                    
                dot.append('}')
                st.graphviz_chart("\n".join(dot))
            except Exception as e:
                st.error(f"无法渲染流程图: {e}")
        ...
        if adv_mode:
            ...
            # Visual Preview
            with st.expander("👁️ 流程图预览 (Flowchart)", expanded=True):
                _render_stage_flowchart(struct)

            cols = st.columns(2)
            with cols[0]:
                with st.expander("📊 节点列表", expanded=True):
                    st.table(struct.get("nodes") or [])
                
                with st.expander("➕ 添加/更新节点", expanded=False):
            ...
            with cols[1]:
                with st.expander("🔀 跳转规则", expanded=True):
                    st.table(struct.get("transitions") or [])
                
                with st.expander("➕ 添加跳转", expanded=False):
            ...
            if ok and st.button("保存为新版本", width="stretch", key="btn_save_struct"):
                ...
                st.success("已保存")
            st.divider()
            with st.expander("📜 历史版本 (History)", expanded=False):
                st.markdown("历史版本")
    ...
    with tabs[1]:
        ...
        with st.expander("⚙️ Persona 基础设置", expanded=True):
            presets = [
        ...
        if adv_mode_p:
            ...
            with st.expander("👁️ 实时预览与保存", expanded=True):
    ...
    with tabs[2]:
        ...
        with st.expander("⚙️ 绑定规则编辑器", expanded=True):
    ...
    with tabs[3]:
        ...
        with st.expander("🧪 模拟参数设置", expanded=True):
        ...
        if st.button("执行模拟", width="stretch", key="sim_run"):
            ...
            with st.expander("📊 模拟结果", expanded=True):
                st.json(res)
    ...
    with tabs[4]:
        ...
        with st.expander("📝 测试用例集", expanded=True):
    ...
    with tabs[5]:
        with st.expander("🛡️ 风格守卫说明 (Style Guard Overview)", expanded=True):
            st.markdown("### 🛡️ 风格守卫 (Regex Guard)")
        ...
        with st.expander("🛡️ 规则配置", expanded=True):
    ...
    with tabs[6]:
        ...
        with st.expander("🩺 开始检查", expanded=True):
...
def render_supervisor_panel():
    ...
    with tab1:
        # st.subheader(tr("sup_list_title")) # Redundant with expander
        ...
        with st.expander("👤 用户与上下文管理", expanded=True):
        ...
        with st.expander("🛠️ 状态干预 (Advanced)", expanded=False):
    ...
    with tab2:
        # st.subheader(tr("sup_route_title")) # Redundant with Tab label
...
def render_ai_config_panel():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🤖 模型注册表 (Model Registry)", "⛓️ 会话智能体配置 (Session Agent Pipeline)", "📚 RAG 知识库配置 (Knowledge Base)", "🛠️ 技能中心 (Skill Center)"])

    # --- Tab 1: Model Registry ---
    with tab1:
        st.caption(tr("acc_tenant").format(tenant))
        
        with st.expander("➕ 添加/编辑模型 (Add/Edit Model)", expanded=True):
        ...
        st.divider()
        with st.expander("📋 已配置模型列表 (Registered Models)", expanded=True):
            disp_providers = []
    ...
    with tab2:
        ...
        # 1. Supervisor
        with st.expander("1. 🧠 场控 (Supervisor)", expanded=True):
        ...
        with st.expander("2. 🎭 执行者 (Worker / Stage Agent)", expanded=True):
            c1, c2 = st.columns([1, 3])
            with c1:
        ...
        # 3. Audit Primary
        with st.expander("3. 🛡️ 初审 (Audit Primary)", expanded=True):
            c1, c2 = st.columns([1, 3])
            with c1:
        ...
        # 4. Audit Secondary
        with st.expander("4. ⚖️ 复审 (Audit Secondary)", expanded=True):
            c1, c2 = st.columns([1, 3])
            with c1:
    ...
    # --- Tab 3: RAG Knowledge Base Config ---
    with tab3:
        st.caption("在此配置 RAG (Retrieval-Augmented Generation) 知识库的参数，包括向量模型、混合检索策略等。")
        
        # Load RAG config
        rag_conf_path = os.path.join(tdir, "platforms", "telegram", "rag_config.json")
        rag_conf = {}
        try:
            if os.path.exists(rag_conf_path):
                with open(rag_conf_path, "r", encoding="utf-8") as f:
                    rag_conf = json.load(f)
        except Exception:
            pass

        # 1. Embedding Model
        st.subheader("1. 向量模型 (Embedding Model)")
        st.info("选择用于生成知识库向量的模型。注意：切换模型后需要重建索引。")
        
        # Filter models suitable for embeddings (usually OpenAI or LocalAI)
        # For now, we list all registered models, but user should choose wisely.
        # Ideally we should filter by provider capabilities if metadata existed.
        embed_model_opts = [("", "默认 (text-embedding-3-small)")] + model_opts
        
        cur_embed_model = rag_conf.get("embedding_model_id", "")
        # Try to match current config to option index
        embed_idx = 0
        for i, (k, _) in enumerate(embed_model_opts):
            if k == cur_embed_model:
                embed_idx = i
                break
        
        sel_embed_model = st.selectbox("选择 Embedding 模型", embed_model_opts, index=embed_idx, format_func=lambda x: x[1], key="rag_embed_model")
        
        # 2. Retrieval Strategy
        st.subheader("2. 检索策略 (Retrieval Strategy)")
        c1, c2 = st.columns(2)
        with c1:
            top_k = st.number_input("Top K (返回条数)", min_value=1, max_value=20, value=int(rag_conf.get("top_k", 3)), key="rag_top_k")
        with c2:
            score_threshold = st.slider("相似度阈值 (Score Threshold)", 0.0, 1.0, float(rag_conf.get("score_threshold", 0.45)), 0.05, key="rag_threshold")
            
        hybrid_weight = st.slider("混合检索权重 (Hybrid Weight)", 0.0, 1.0, float(rag_conf.get("hybrid_weight", 0.7)), 0.1, help="向量检索分数占比。1.0表示纯向量，0.0表示纯关键词。")
        
        # 3. Save Config
        if st.button("💾 保存 RAG 配置", key="save_rag_conf"):
            rag_conf["embedding_model_id"] = sel_embed_model[0]
            rag_conf["top_k"] = top_k
            rag_conf["score_threshold"] = score_threshold
            rag_conf["hybrid_weight"] = hybrid_weight
            
            try:
                os.makedirs(os.path.dirname(rag_conf_path), exist_ok=True)
                with open(rag_conf_path, "w", encoding="utf-8") as f:
                    json.dump(rag_conf, f, ensure_ascii=False, indent=2)
        ...
        # 4. Reindex Action
        st.subheader("3. 索引管理 (Index Management)")
        st.warning("⚠️ 更改 Embedding 模型后，必须重建索引。重建过程可能需要几分钟（取决于知识库大小）。")
        
        if st.button("🔄 立即重建向量索引 (Reindex)", type="primary", key="btn_reindex"):
            with st.status("正在重建索引...", expanded=True) as status:
                st.write("正在启动索引工具...")
                try:
                    # Run reindex_kb.py as subprocess
                    import subprocess
                    cmd = [sys.executable, "tools/reindex_kb.py", "--tenant", tenant]
                    result = subprocess.run(cmd, capture_output=True, text=True)
                    
                    if result.returncode == 0:
                        st.write("✅ 工具执行成功")
                        st.code(result.stdout)
                        status.update(label="重建索引完成！", state="complete", expanded=False)
                        st.success("索引重建完成！请重启机器人以加载新索引。")
                    else:
                        st.write("❌ 工具执行失败")
                        st.code(result.stderr)
                        status.update(label="重建索引失败", state="error")
                except Exception as e:
                    st.error(f"执行出错: {e}")

    # --- Tab 4: Skill Center ---
    with tab4:
        st.caption("在此管理 AI 可调用的工具 (Function Calling)。启用后，AI 可以根据用户意图自动调用这些工具。")
        
        # List Skills
        skills = db.list_skills(tenant)
        
        # Scan and Sync Built-in Skills to DB (if not exists)
        from src.modules.knowledge_base.skill_center import skill_registry
        builtin_tools = skill_registry.get_tools_definition() # Get all registered in code
        for t in builtin_tools:
            fname = t["function"]["name"]
            fdesc = t["function"]["description"]
            # Check if exists
            exists = any(s["name"] == fname for s in skills)
            if not exists:
                db.upsert_skill(tenant, {
                    "name": fname,
                    "description": fdesc,
        ...
        if not skills:
            ...
            for s in skills:
                with st.expander(f"{'✅' if s['enabled'] else '🚫'} {s['name']} - {s['description']}", expanded=False):
                    with st.form(key=f"skill_form_{s['id']}"):
                        c1, c2 = st.columns([1, 3])
                        with c1:
                            en = st.toggle("启用 (Enabled)", value=bool(s['enabled']), key=f"skill_en_{s['id']}")
                        with c2:
                            bound_ai = st.selectbox("绑定特定AI (可选)", [""] + [k for k, _ in model_opts], 
                                                  index=_get_idx(s.get("bound_ai_id", "")), 
                                                  format_func=lambda x: "通用 (所有模型)" if x=="" else x,
                                                  key=f"skill_ai_{s['id']}")
                        
                        # Config Editor (JSON)
                        conf_str = json.dumps(s.get("config", {}), indent=2, ensure_ascii=False)
                        new_conf = st.text_area("参数配置 (JSON)", value=conf_str, height=100, help="例如 API Key 等配置", key=f"skill_conf_{s['id']}")
                        
                        c3, c4 = st.columns(2)
                        with c3:
                            if st.form_submit_button("💾 保存配置"):
                                try:
                                    conf_obj = json.loads(new_conf)
                                    db.upsert_skill(tenant, {
                                        "id": s["id"],
                                        "name": s["name"],
                                        "description": s["description"],
                                        "enabled": en,
                                        "bound_ai_id": bound_ai,
                                        "config": conf_obj
                                    })
                                    st.success("保存成功")
                                    st.rerun()
                    ...
                    if st.button("🗑️ 删除技能", key=f"btn_del_skill_{s['id']}"):
                        db.delete_skill(tenant, s['id'])
                        st.success("已删除")
                        st.rerun()
...
def render_ai_learning_panel():
    ...
    with tab_browse:
        ...
        if not rows:
            ...
            with st.expander("📊 数据列表与批量操作", expanded=True):
                import pandas as pd
                view = []
                for r in rows:
                    view.append({
    ...
    with tab_export:
        with st.expander("📦 导出选项", expanded=True):
...
def render_skills_panel():
    ...
    with tab_edit:
        ...
        with st.expander("🔍 选择技能", expanded=True):
...
def render_api_gateway_panel():
    ...
    with st.expander(tr('api_btn_add'), expanded=True):
        ...
        if st.button(tr('api_btn_add'), width="stretch", key="gw_add"):
            ...
            st.rerun()
    st.divider()
    # st.markdown(tr('api_list_header')) # Moved to expander title
    disp_routes = []
    ...
    with st.expander("📋 " + tr('api_list_header'), expanded=True):
        if not disp_routes:
            st.info("暂无路由配置")
        else:
...
def render_sys_config_panel():
    ...
    with colA:
        with st.expander(tr('sys_env_header'), expanded=True):
    ...
    with colB:
        with st.expander(tr('sys_session_header'), expanded=True):
            ...
            if st.button(tr('sys_btn_init_session'), width="stretch", key="btn_init_sessions"):
                try:
                    ...
                    st.error(f"初始化失败: {e}")
    st.divider()
    with st.expander("🔒 敏感信息管理", expanded=True):
    ...
    if st.session_state.get("user_role") == "SuperAdmin" and VersionManager:
        with st.expander("📦 版本管理与回滚 (Beta)", expanded=True):
            st.caption("切换版本将修改启动配置。")
            vm = VersionManager(BASE_DIR)
            current_v = vm.get_current_version()
            
            versions = vm.list_versions()
            try:
                current_idx = versions.index(current_v)
            except ValueError:
                current_idx = 0

            col_v1, col_v2 = st.columns([1, 2])
            with col_v1:
                st.metric("当前版本", current_v)
            with col_v2:
                target_v = st.selectbox("选择目标版本", versions, index=current_idx)
                
                if st.button("🚀 切换版本", type="primary"):
                    if target_v == current_v:
                        st.warning("已是当前版本")
                    else:
                        success, msg = vm.switch_version(target_v)
                        if success:
                            st.success(f"{msg}")
                            # log op
                            sys_user = st.session_state.get("sys_user", {})
                            log_admin_op("version_switch", {
                                "from": current_v, 
                                "to": target_v, 
                                "user": sys_user.get("username", "unknown")
                            })
                        else:
                            st.error(msg)
...
def render_business_panel():
    ...
    with tab1:
        ...
        with col_chart1:
            with st.expander(f"📈 {tr('bus_trend')}", expanded=True):
        ...
        with col_chart2:
            with st.expander(f"💰 {tr('bus_cost_breakdown')}", expanded=True):
        ...
        with st.expander(f"📉 {tr('bus_funnel')}", expanded=True):
    ...
    with tab2:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            with st.expander(f"### {tr('bus_plan_free_title')}", expanded=True):
    ...
    with tab3:
        ...
        with st.expander("🎨 品牌设置", expanded=True):
...
def render_telegram_stats():
    """Telegram 统计界面 (多租户适配版)"""
    st.subheader("📊 使用统计")
    
    tenant_id = st.session_state.get('tenant', 'default')
    paths = _get_tenant_tg_paths(tenant_id)
    stats_file = paths["stats"]

    # 兼容旧路径
    if tenant_id == 'default' and not os.path.exists(stats_file) and os.path.exists("platforms/telegram/stats.json"):
    ...
    try:
        ...
        if stats.get('total_replies', 0) > 0:
            success_rate = (stats.get('success_count', 0) / stats.get('total_replies', 1)) * 100
        ...
        with st.expander("📊 核心指标", expanded=True):
            ...
            with col2:
                st.metric("总回复数", stats.get('total_replies', 0))
            
            with col3:
                st.metric("成功率", f"{success_rate:.1f}%")
            
            with col4:
                st.metric("失败次数", stats.get('error_count', 0))
            ...
            with col1:
                st.metric("私聊消息", stats.get('private_messages', 0))
            
            with col2:
                st.metric("群聊消息", stats.get('group_messages', 0))
        ...
        # 审核与兜底趋势（最近 1000 行）
        try:
            audit_path = os.path.join(paths["logs_dir"], "audit.log")
            if tenant_id == 'default' and not os.path.exists(audit_path):
                 audit_path = "platforms/telegram/logs/audit.log"
            ...
            with st.expander("📊 审核与兜底趋势", expanded=True):
...
def render_whatsapp_config():
    ...
    with st.expander(tr('wa_cfg_persona'), expanded=True):
        col1, col2 = st.columns(2)
    ...
    with st.expander(tr('wa_cfg_switches'), expanded=True):
...
def render_whatsapp_logs():
    ...
    with st.expander("📜 日志监控", expanded=True):
...
def render_whatsapp_stats():
    ...
    try:
        ...
        # 显示统计
        with st.expander(f"📊 {tr('wa_stats_header')}", expanded=True):
            col1, col2, col3, col4 = st.columns(4)
...
def render_audit_panel():
    ...
    with tab1:
        ...
        with col1:
            with st.expander(tr("audit_block_header"), expanded=True):
        ...
        with col2:
            with st.expander(tr("audit_sens_header"), expanded=True):
        ...
        st.divider()
        with st.expander(tr("audit_allow_header"), expanded=True):
    ...
    with tab2:
        # st.subheader(tr("audit_tab_logs")) # Moved to expander
        # 从数据库加载日志
        tenant_id = st.session_state.get("tenant", "default")
        ...
        # 优先使用数据库
        with st.expander(f"📜 {tr('audit_tab_logs')}", expanded=True):
            try:
    ...
    with tab3:
        ...
        # 6. 配置导入/导出
        with st.expander("📦 维护与迁移", expanded=False):
        ...
        # 7. 备份与回滚
        with st.expander("🔄 备份与回滚", expanded=False):
        ...
        # 二次验证机制（简单验证码）
        with st.expander("🔐 安全验证", expanded=True):
...
def render_system_admin_panel():
    ...
    with tabs[0]:
        c1, c2 = st.columns(2)
        with c1:
            # st.subheader("租户") # Moved to expander
            with st.expander("🏢 租户列表", expanded=True):
                tenants = []
                try:
                ...
                if ok:
                    if not tid:
                        ...
                        try:
                            ...
                            st.error(str(e))
        with c2:
            # st.subheader("系统账号") # Moved to expander
            with st.expander("👤 系统账号列表", expanded=True):
                users = []
                try:
    ...
    with tabs[1]:
        # st.subheader("IP白名单") # Moved to expander
        ips = []
        try:
    ...
    with tabs[2]:
        ...
        with st.expander("📜 登录日志列表", expanded=True):
            try:
    ...
    with tabs[4]:
        ...
        # 1. 本地版本状态
        with st.expander("📦 版本状态", expanded=True):
            st.info(f"当前运行版本: **{APP_VERSION}**")
            
            # Show local versions
            try:
                vm = VersionManager(BASE_DIR)
                versions = vm.list_versions()
                st.write(f"本地已安装版本: {', '.join(versions)}")
            except:
                st.warning("版本管理器未初始化")

        st.divider()

        # 2. GitHub Online Upgrade
        st.subheader("☁️ 在线升级 (GitHub)")
        st.caption("从 GitHub 仓库直接下载并部署新版本。")
        
        col_repo, col_btn = st.columns([3, 1])
        with col_repo:
            # Default repo
            repo_default = db.get_system_config("github_repo", "SaaS-AIs/SaaS-AI-System")
            repo_input = st.text_input("GitHub Repository", value=repo_default, help="格式: Owner/Repo", key="gh_repo_input")
        with col_btn:
            st.write("") 
            st.write("") 
            check_click = st.button("🔍 检查更新", type="primary", key="gh_check_btn")
            
        if check_click and repo_input:
            db.set_system_config("github_repo", repo_input)
            try:
                with st.spinner(f"正在连接 {repo_input}..."):
                    # Use GitHub API
                    api_url = f"https://api.github.com/repos/{repo_input}/releases/latest"
                    # Add headers to avoid rate limits if possible, but anonymous is fine for low volume
                    ...
                    if r.status_code == 200:
                        ...
                        st.success(f"发现最新版本: {tag_name}")
                        st.markdown(f"[查看 Release 页面]({html_url})")
                        
                        with st.expander("📄 更新日志", expanded=True):
                            st.markdown(body)
                            
                        # Find zip asset
                        zip_url = data.get("zipball_url") # Fallback
                        asset_url = None
                        asset_name = ""
                        for a in assets:
                            if a.get("name", "").endswith(".zip") and "SaaS-AI" in a.get("name", ""):
                                asset_url = a.get("browser_download_url")
                                asset_name = a.get("name")
                                break
                        
                        final_url = asset_url or zip_url
                        
                        st.warning(f"准备下载: {asset_name if asset_name else 'Source Code (zip)'}")
                        
                        if st.button(f"⬇️ 下载并安装 {tag_name}", key="gh_install_btn"):
                             with st.status("正在安装更新...", expanded=True) as status:
                                try:
                                    st.write(f"正在下载安装包... ({final_url})")
                                    r_zip = requests.get(final_url, stream=True, timeout=60)
                                    z = zipfile.ZipFile(io.BytesIO(r_zip.content))
                                    
                                    # Determine target directory name
                                    target_dir_name = tag_name
                                    if not target_dir_name.startswith("v"):
                                        target_dir_name = f"v{tag_name}"
                                        
                                    target_path = os.path.join(BASE_DIR, "releases", target_dir_name)
                                    
                                    st.write(f"正在解压至 {target_path}...")
                                    
                                    # Extract to temp
                                    with tempfile.TemporaryDirectory() as tmpdirname:
                                        z.extractall(tmpdirname)
                                        
                                        # Handle root folder in zip (GitHub usually wraps in repo-tag)
                                        items = os.listdir(tmpdirname)
                                        if len(items) == 1 and os.path.isdir(os.path.join(tmpdirname, items[0])):
                                            src_content = os.path.join(tmpdirname, items[0])
                                        else:
                                            src_content = tmpdirname
                                            
                                        # Copy to target
                                        if os.path.exists(target_path):
                                            st.write("清理旧版本目录...")
                                            shutil.rmtree(target_path)
                                        shutil.copytree(src_content, target_path)
                                    
                                    st.write("正在更新启动配置...")
                                    vm = VersionManager(BASE_DIR)
                                    success, msg = vm.switch_version(target_dir_name)
                                    
                                    if success:
                                        status.update(label="✅ 升级成功！", state="complete", expanded=False)
                                        st.success(f"已安装并切换至 {target_dir_name}。请立即重启服务。")
                                        st.balloons()
                                    else:
                                        status.update(label="❌ 切换版本失败", state="error")
                                        st.error(msg)
                                        
                                except Exception as e:
                                    status.update(label="❌ 安装失败", state="error")
                                    st.error(f"详细错误: {str(e)}")
                                    traceback.print_exc()
                    elif r.status_code == 404:
                        st.warning("未找到 Release 发布版本，请检查 Repo 名称。")
                    else:
                        st.error(f"无法获取版本信息: {r.status_code} - {r.text}")
            except Exception as e:
                st.error(f"连接失败: {e}")
