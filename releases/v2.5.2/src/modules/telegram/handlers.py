...
def register_handlers(app):
    ...
    async def handler(event):
        ...
        is_filtered, trigger_kw = check_content_filter(msg, config)
        if is_filtered:
            app.logger.log_system(f"🛡️ 内容过滤触发: keyword='{trigger_kw}', msg='{msg}'")
            # Optional: Record event as filtered
            try:
                app.db.record_message_event(app.tenant_id, "telegram", str(event.chat_id), "inbound", "filtered", user_content=msg, bot_response=f"Filtered by {trigger_kw}")
            except:
                pass
            return
        ...
        async with app.client.action(event.chat_id, 'typing'):
            ...
            if not orch_enabled and not kb_context:
                kb_hits = app.kb_engine.retrieve_kb_context(msg, kb_items, topn=2, tenant_id=app.tenant_id)
                if kb_hits:
