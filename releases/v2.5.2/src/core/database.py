...
class DatabaseManager:
    ...
    def _migrate_tables(self):
        ...
        try:
            ...
            # Index Optimization (V2.5.2)
            # Create indexes if they don't exist
            # Note: "IF NOT EXISTS" is supported in recent SQLite versions.
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_msg_tenant_time ON message_events(tenant_id, timestamp DESC)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_msg_learning ON message_events(tenant_id, is_learnable, is_junk)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_msg_chat ON message_events(chat_id, timestamp)")
...
if os.environ.get("DATABASE_URL") and os.environ.get("DATABASE_URL").startswith("postgres"):
    try:
        from .database_pg import DatabaseManagerPG
        print("🔌 Using PostgreSQL Database")
        db = DatabaseManagerPG()
    except ImportError as e:
        print(f"⚠️ PostgreSQL driver not found, falling back to SQLite: {e}")
        db = DatabaseManager()
    except Exception as e:
        print(f"⚠️ Failed to initialize PostgreSQL, falling back to SQLite: {e}")
        db = DatabaseManager()
else:
    db = DatabaseManager()
