...
class AIClientManager:
    def __init__(self, api_key, base_url, logger):
        ...
        self.ai_client = None
        self.sync_client = None # For synchronous operations (like KB embedding)
    ...
    def get_sync_client(self):
        """Get a synchronous OpenAI client for blocking operations"""
        if self.sync_client is None:
            ssl_mode = self._ssl_verify_default()
            # Use standard httpx Client for sync
            http_client = httpx.Client(verify=ssl_mode, timeout=30.0)
